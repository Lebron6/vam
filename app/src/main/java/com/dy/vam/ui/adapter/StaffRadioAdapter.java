...
public class StaffRadioAdapter extends BaseAdapter implements SectionIndexer{


	private List<SortModelInfo> list = null;
	private Context mContext;
	private int index=-1;
	private OnStaffCheckedListener onStaffCheckedListener;

	public StaffRadioAdapter(Context mContext, List<SortModelInfo> list, OnStaffCheckedListener onStaffCheckedListener) {
		this.mContext = mContext;
		this.list = list;
		this.onStaffCheckedListener = onStaffCheckedListener;
	}
	...
	public View getView(final int position, View view, ViewGroup arg2) {
		...
		if (view == null) {
			...
			view = LayoutInflater.from(mContext).inflate(R.layout.item_staff_radio, null);
		...
		viewHolder.checkBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                if (finalViewHolder.checkBox.isChecked()){
					onStaffCheckedListener.onCheckedCheckenListener(position);
                    index=position;
		...
		if (list.get(position).type==list.get(position).TYPE_CHECKED) {// 选中的条目和当前的条目是否相等
