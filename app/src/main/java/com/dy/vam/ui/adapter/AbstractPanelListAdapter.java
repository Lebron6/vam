...
import com.dy.vam.ui.widget.MyHorizontalScrollView;
...
public abstract class AbstractPanelListAdapter {
     ...
     * 初始化总Adapter，加载数据到视图
     */
    public void initAdapter(AdapterView.OnItemClickListener onItemClickListener) {
        ...
        lv_column.setOnItemClickListener(onItemClickListener);
    }
...
//        pl_root.removeView(lv_content);
        Utils.removeSelfFromParent(lv_content);
        // 1. title (TextView --> PanelListLayout)
        tv_title = new TextView(context);
        tv_title.setText(title);
        tv_title.setTextColor(context.getResources().getColor(R.color.colorTextTheme));
        if (titleBackgroundResource != 0) {
            tv_title.setBackgroundResource(titleBackgroundResource);
        }
//        tv_title.getPaint().setFakeBoldText(true);
        tv_title.setGravity(Gravity.CENTER | Gravity.CENTER_VERTICAL);
...
//        lv_column.setPadding(10,0,0,0);
        lv_column.setDivider(null);
...
//                ll_contentItem = (LinearLayout) lv_content.getChildAt(lv_content.getFirstVisiblePosition());//获得content的第一个可见item
    ...
    private void initRowLayout() {
        ...
        for (int i = 0; i < rowCount; i++) {
            ...
            rowItem.setTextColor(context.getResources().getColor(R.color.colorTextTheme));
...
//            columnAdapter = new ColumnAdapter(context, R.layout.item_column, getColumnDataList());
//        }
...
//                Log.e("TitleWidth:",titleWidth+"----");
//                Log.e("columnItemHeight:",columnItemHeight+"----");
//                AbsListView.LayoutParams lp = new AbsListView.LayoutParams(titleWidth, columnItemHeight);
