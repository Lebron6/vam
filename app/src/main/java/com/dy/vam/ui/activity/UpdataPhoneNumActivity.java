...
public class UpdataPhoneNumActivity extends BaseActivity {
    ...
    public static void actionStart(Context context, String title) {
        Intent intent = new Intent(context, UpdataPhoneNumActivity.class);
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.tv_get_phone_code:
                account=etOldPhoneNum.getText().toString();
                if (TextUtils.isEmpty(account)) {
                    Toasty.error(UpdataPhoneNumActivity.this, "获取验证码手机号为空").show();
                    return;
                }
                getPhoneCode();
                break;
            case R.id.btn_next:
                ...
                if (TextUtils.isEmpty(code)) {
                    Toasty.error(UpdataPhoneNumActivity.this, "请输入验证码").show();
                    return;
                }
                toUpdataPhoneNum(code);
                break;
        }
    }

    private void toUpdataPhoneNum(String code) {
        btnNext.setEnabled(false);
        LoadDialog.show(UpdataPhoneNumActivity.this);
        createRequest(BaseUrl.getInstence().getBindNewPhoneUrl()).bindNewPhoneNew(PreferenceUtils.getInstance().getUserToken(),account, code).enqueue(new Callback<Verif>() {

            @Override
            public void onResponse(Call<Verif> call, Response<Verif> response) {
                btnNext.setEnabled(true);
                LoadDialog.dismiss(UpdataPhoneNumActivity.this);
                switch (response.body().getCode()) {
                    case Constant.SUCCESS:
                        Toasty.info(UpdataPhoneNumActivity.this,"手机号修改成功").show();
                        finish();
                        break;
                    default:
                        Toasty.error(UpdataPhoneNumActivity.this, response.body().getMsg()).show();
                        break;
                }
            }
            ...
            public void onFailure(Call<Verif> call, Throwable t) {
                LoadDialog.dismiss(UpdataPhoneNumActivity.this);
                btnNext.setEnabled(true);
                Toasty.warning(UpdataPhoneNumActivity.this, getString(R.string.net_error)).show();
...
LoadDialog.show(this);
        createRequest(BaseUrl.getInstence().getSendMsgUrl()).sendMsg(account, Utils.getIMEI(this), "3").enqueue(new Callback<Verif>() {
            @Override
            public void onResponse(Call<Verif> call, Response<Verif> response) {
                LoadDialog.dismiss(UpdataPhoneNumActivity.this);
                switch (response.body().getCode()) {
                    case Constant.SUCCESS:
                        ...
                        time.start();// 开始计时
                        Toasty.info(UpdataPhoneNumActivity.this, "请查收验证码").show();
                        break;
                    default:
                        Toasty.error(UpdataPhoneNumActivity.this, response.body().getMsg()).show();
                        break;
            ...
            @Override
            public void onFailure(Call<Verif> call, Throwable t) {
                Toasty.warning(UpdataPhoneNumActivity.this, "网络异常!").show();
